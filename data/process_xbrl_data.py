...
def process_finer139_dataset(dataset_name):
    for split in ['train', 'test']:
        dataset = load_dataset(dataset_name, split=split)  # Use only train split for this example
        dataset = dataset.shuffle()
        all_data = []
        for example in tqdm(dataset):
            processed = process_example(example, labels)
            if processed is not None:
                all_data += processed
            if len(all_data) > 100e3:
                break


        with open(f"{split}/finer_{split}.jsonl", 'w') as writer:
            for x in all_data:
...
def process_example(example, labels):
    ...
    for i, x in zip(ner_tok_idx, ner_tag_idx):
        ...
        processed_example = {
            "context": f'What is the appropriate XBRL US GAAP tag for "{tokens[i]}" in the given sentence? Output the '
                       f'US GAAP tag only and nothing else. \n "' + sentence +
                       '"\n',
            "target": target,
        }
